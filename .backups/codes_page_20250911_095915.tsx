...
export default function CodesPage(){
  ...
  const issues = useMemo(()=>{
    const missingSupplier = rows.filter(r=>!r.supplier).map(r=>r.id);
    const missingType = rows.filter(r=>!r.codeType).map(r=>r.id);
    const count = missingSupplier.length + missingType.length;
    return {count, missingSupplier, missingType};
  },[rows]);

  const copy=(t:string)=>{ try{ navigator.clipboard?.writeText(t);}catch{} };
  const copyAll=()=>{ const all = built.map(b=>b.code).join('\n\n'); try{ navigator.clipboard?.writeText(all);}catch{} };
    ...
    <div className={`${styles.theme} p-6 space-y-4`}>
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <a href="/generation" className={`${styles.btn} ${styles.btnGhost}`}>← Назад к Генерации</a>
          <button onClick={copyAll} className={`${styles.btn} ${styles.btnGhost}`}>Скопировать все</button>
          ...
          <tbody>
            {built.map(({row,code,kind,warn})=>(
              <tr key={row.id} className="border-t hover:bg-slate-50 text-sm">
                ...
                <td className="px-3 py-2">
                  <button onClick={()=>copy(code)} className={`${styles.btn} ${styles.btnGhost}`}>Скопировать</button>
                </td>
