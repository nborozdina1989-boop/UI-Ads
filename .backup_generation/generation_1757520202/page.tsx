...
/** ЛОКАЛЬНЫЕ КОНСТАНТЫ — НИКУДА НЕ ИМПОРТИРУЕМ */
const CODE_TYPES = [
...
/** ЛОКАЛЬНЫЕ МЕЛКИЕ КОМПОНЕНТЫ */
function Tabs({active}:{active:'gen'|'codes'}) {
  const base='/generation';
  const cls="px-3 py-1.5 rounded-md border text-sm";
  return (
    <div className="flex gap-2">
      <a href={base} className={`${cls} ${active==='gen'?'bg-slate-900 text-white':'bg-white'}`}>Генерация</a>
      <a href={`${base}/codes`} className={`${cls} ${active==='codes'?'bg-slate-900 text-white':'bg-white'}`}>Коды</a>
    </div>
  );
}
            ...
            {filtered.map(r=>{
              ...
              return (
                <tr key={r.id} className="hover:bg-slate-50">
                  ...
                  <td className={td}>
                    <div className="flex items-center gap-2">
                      <button onClick={()=>detect(i)} className="text-xs px-2 py-0.5 border rounded">авто</button>
                      <span>{r.title}</span>
                    </div>
                  </td>
                  {vis.supplier && (
